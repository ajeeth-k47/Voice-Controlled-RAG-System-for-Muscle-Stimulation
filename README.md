...
With further development, this prototype can be integrated with hardware (an EMS device) to help physiotherapists or wearable EMS systems for movements, without requiring manual EMS parameter configuration every time.
...
During the implementation of the system, I used Visual Studio Code with the GitHub Copilot plugin, which helped with code rewriting and suggestions for writing cleaner code. I also used the OpenSim community developed Python packages and API for Integrating the openSim, generating glTF format(efficient format for web) and setting activation and joint values which is generated by LLM reponse for the user asked movement.